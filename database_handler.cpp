...
void DatabaseHandler::aggregate(const std::string &table, size_t searchColumn, const std::string &searchValue, size_t targetColumn, const std::string &operation)
{
    ...
    if (searchTable != nullptr)
    {
        ...
        else
        {
            if (operation == "sum")
            {
                double result = sum(*searchTable, searchValue, searchColumn, targetColumn);

                std::cout << "The sum is: " << result << '\n';
            }
            else if (operation == "product")
            {
                double result = product(*searchTable, searchValue, searchColumn, targetColumn);
                std::cout << "The product is: " << result << '\n';
            }
            else if (operation == "maximum")
            {
                double result = maximum(*searchTable, searchValue, searchColumn, targetColumn);
                std::cout << "The maximum value is: " << result << '\n';
            }
            else if (operation == "minimum")
            {
                double result = minimum(*searchTable, searchValue, searchColumn, targetColumn);
...
double DatabaseHandler::sum(const Table &table, const std::string &value, size_t searchColumn, size_t targetColumn) const
{
    double sum = 0;

    for (size_t i = 0; i < table[searchColumn - 1]->size(); i++)
    {
        if ((*(table[searchColumn - 1]))[i] == value)
        {

            if (table[targetColumn - 1]->type() == "integer")
            {
                const IntColumn *col = dynamic_cast<const IntColumn *>(table[targetColumn - 1]);
                sum += col->valueAsNumber(i);
            }
            else if (table[targetColumn - 1]->type() == "double")
            {
                const DoubleColumn *col = dynamic_cast<const DoubleColumn *>(table[targetColumn - 1]);
                sum += col->valueAsNumber(i);
            }
        }
    }
...
double DatabaseHandler::product(const Table &table, const std::string &value, size_t searchColumn, size_t targetColumn) const
{
    double product = 1;

    for (size_t i = 0; i < table[searchColumn - 1]->size(); i++)
    {
        if ((*(table[searchColumn - 1]))[i] == value && (*(table[targetColumn - 1]))[i] != "NULL")
        {
...
double DatabaseHandler::maximum(const Table &table, const std::string &value, size_t searchColumn, size_t targetColumn) const
{
...
double DatabaseHandler::minimum(const Table &table, const std::string &value, size_t searchColumn, size_t targetColumn) const
{
...
void DatabaseHandler::visualizationInfo() const
{
